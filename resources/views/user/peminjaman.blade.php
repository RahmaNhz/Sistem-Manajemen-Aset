      ...
      <div class="container">
          <div class="row justify-content-center">
              <div class="col-8">
                  <div class='card'>
                      <div class="card-body">
                          <form action="{{ route('simpanpeminjaman') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @if(session('error'))
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="namaset" name="aset_id" value="{{ $dataaset->nama_aset }}" readonly>
                                <input type="hidden" name="aset_id" value="{{ $dataaset->id }}">
                            </div>

                            <div class="mb-3">
                                <label for="nama_ruang" class="form-label">Nama Ruang</label>
                                <input type="text" class="form-control" id="nama_ruang" name="room_id" value="{{ $dataaset->room->nama_room }}" readonly>
                            </div>

                            <div class="mb-3">
                                <label for="nama_peminjam" class="form-label">Nama Peminjam</label>
                                <input type="text" class="form-control" id="nama_peminjam" name="pegawai_id" value="{{ $user->nama }}" readonly>
                            </div>

                            <div class="mb-3">
                                <label for="nik_peminjam" class="form-label">NIK</label>
                                <input type="text" class="form-control" id="nik_peminjam" name="nik_peminjam" value="{{ $user->nik}}"readonly >
                            </div>
