    ...
    <div class="row row-cols-1 row-cols-md-5 g-4">
      ...
      <div class="col">
        <div class="card" style="width: 18rem;">
          <img src="{{ asset('fotoaset/'.$row->foto) }}" class="card-img-top" alt="...">
          <div class="card-body">
            <h5 class="card-title"><b>{{ $row->nama_aset }}</b></h5>
            <p class="card-text">Kategori: {{ $row->kategori->nama_kategori }}</p>
            <p class="card-text">Stock: {{ $row->jumlah_aset }}</p>
            <p class="card-text">Ruang: {{ $row->room->nama_room }}</p>
            <a href="/pegawai/tampil/{{ $row->id }}" class="btn btn-primary">Pinjam</a>
          </div>
        </div>
      </div>
      @endforeach
